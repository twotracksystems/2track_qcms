...
export default function AddOrderList(params:any) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(`/api/v1/getonearticlemax/?id=${id}`);
      if (!response.ok) {
  ...
  const updateUserMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await fetch(`/api/v1/edit_article_max?id=${id}`, {
        method: "PUT",
        headers: {
    ...
    onSuccess: (data) => {
      ...
      router.push("/dashboard/article_max_management");
    },
    onMutate: (data) => {
      return data;
    },
  ...
  return (
    <div className="flex flex-col w-11/12 mx-auto text-black">
      <div className="breadcrumbs my-4 text-lg text-slate-600 font-semibold">
        <ul>
          <li>
            <Link href="/dashboard/article_max_management">article Max Management</Link>
          </li>
          <li>
            <span>Edit Article Max</span>
          </li>
        </ul>
      </div>
      <Formik
