...
interface DownloadOrderListViewProps {
	...
	tableName?: string; // defaults to 'orders'
	ofId?: string | number;
	customerName?: string;
}
...
export default function DownloadOrderListView({
	...
	const handleClick = async () => {
		try {
			// Use existing app APIs elsewhere; no Supabase here. Prepare placeholder values.
			const get = (v: any, fallback = "***") => (v === null || v === undefined || v === "" ? fallback : String(v));
			const customer = get(customerName); // populated from parent when a row is selected
			const orderNo = get(ofId);
