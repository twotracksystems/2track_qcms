...
export default function AddOrderList(params: any) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(`/api/v1/getonearticlenominal/?id=${id}`);
      if (!response.ok) {
  ...
  const updateUserMutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await fetch(`/api/v1/edit_article_nominal?id=${id}`, {
        method: "PUT",
        headers: {
    ...
    onSuccess: (data) => {
      ...
      router.push("/dashboard/article_nominal_management");
    },
    onMutate: (data) => {
      return data;
    },
            ...
            <div className="modal-action p-6">
              ...
              >
                {updateUserMutation.isPending ? (
                  ...
                  <>
                    <Pencil /> Edit Article Nominal
                  </>
                )}
              </button>
              <Link
                className="btn btn-ghost btn-md "
                href="/dashboard/article__nominal_management"
              >
                BACK
              </Link>
            </div>
