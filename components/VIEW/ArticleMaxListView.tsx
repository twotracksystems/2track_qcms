  ...
  const { data, isFetching, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/v1/get_article_max?page=${page}&search=${search}&limit=${limit}`,
        {
          method: "GET",
          headers: {
            Accept: "*/*",
    ...
    <div className="overflow-x-auto mt-4 w-11/12 mx-auto text-black">
      <div className="breadcrumbs my-4 text-lg text-slate-600 font-semibold">
        <ul>
          ...
          <li>  
            <span>Article Max Management</span>
          </li>
        </ul>
      </div>
            ...
            {isLoading || isFetching ? (
              ...
              data?.map((get_users: any, index: any) => (
                ...
                <tr key={index}>
                  ...
                  <Link href={`/dashboard/edit_article_max?id=${get_users.id}`} className="link">
